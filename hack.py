...
def predict_score_and_feedback(model, tokenizer, essay_text, device):
    ...
    with torch.no_grad():
        score_prediction = model(input_ids=input_ids, attention_mask=attention_mask)
    # Apply sigmoid function to get values between 0 and 1
    score_prediction = torch.sigmoid(score_prediction)
    # Scale the predicted score to fit within the range of 2 to 10
    scaled_score = (score_prediction * 10).item()
    scaled_score = max(min(scaled_score, 10), 2)  # Ensure score is between 2 and 10
